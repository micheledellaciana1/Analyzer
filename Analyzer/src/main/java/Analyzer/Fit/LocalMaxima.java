...
public class LocalMaxima extends PeakFinder {
    static public LocalMaxima instance = new LocalMaxima();

    protected LocalMaxima() {
        super("LocalMaxima");
    }
    ...
    protected double prominencePeak(int indexPeakPos, XYSeries series) {
        ...
        for (int i = indexPeakPos; i > 1; i--) {
            if (series.getY(i - 1).doubleValue() > series.getY(i).doubleValue()) {
                prominenceLeft = peakHigh - series.getY(i).doubleValue();
                break;
            }
        }
